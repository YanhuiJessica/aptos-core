module aptos_framework::transaction_validation {
    ...
    fun prologue_common(
        ...
        if (
            ...
            if (!is_simulation || txn_authentication_key != ZERO_AUTH_KEY) {
                assert!(
        ...
        if (!is_simulation && gas_payer != @0x0) {
    ...
    fun multi_agent_common_prologue(
        ...
        while ({
            ...
            if (!is_simulation || signer_public_key_hash != ZERO_AUTH_KEY) {
                assert!(
    ...
    fun fee_payer_script_prologue(
        ...
        multi_agent_common_prologue(secondary_signer_addresses, secondary_signer_public_key_hashes, is_simulation);
        if (!is_simulation || fee_payer_public_key_hash != ZERO_AUTH_KEY) {
            assert!(
    ...
    fun fee_payer_script_prologue_collect_deposit(
        ...
        if (!is_simulation && fee_payer_address != @0x0) {
